...
export = function todoUserSteps() {
    ...
    this.Then(/^.* todo list should contain (.*?)$/, function (items: string) {
        return expect(stage.theActorInTheSpotlight().toSee(TodoList.Items_Displayed)).eventually.to.deep.equal(listOf(items));
    });
    ...
    this.Then(/^(?:his|her) todo list contains of (.*?) item left$/, (count: string) => {
        return expect(stage.theActorInTheSpotlight().toSee(Text.of(TodoList.Items_Left_Count))).eventually.to.equal(count);
    });
};
